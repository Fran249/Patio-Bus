<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="6">
               <v-row>
                   <v-col cols="6" v-for="(postre, i) in postres" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectPostre(postre)">
                           <v-img :src="postre.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
                                   {{postre.nombre}}
       ...
       <div class="selectores" >
           <div class="selector"  v-for="postre in postres" :key="postre.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{postres.length}}</h3>
   </div>
   </div>
   </v-container>
</transition>
 ...
 export default {
     ...
     methods: {
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','postres',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.postres = doc.data().pagina
               ...
               this.postres.forEach(item =>{
               getDownloadURL(ref(storage, `Productos/postres/${item.id}.jpg`))
               .then((url) => {
               const newPostre= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newPostres.push(newPostre)
               console.log(this.newPostres)
               if(item.nombre == 'Cassata' || item.nombre == 'Tiramisú' || item.nombre == 'Mouse' || item.nombre == 'Frutillas y crema' || item.nombre == 'Brownie' || item.nombre == 'Don pedro'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','postres','pagina5','pagina');
       setDoc(docRef,{pagina: this.postres});

       },
       selectPostre(postre){
           this.selected = [postre]
           console.log(this.selected)
       },
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','postres','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
       ...
       getDownloadURL(ref(storage, `Productos/postres/${item.id}.jpg`))
           .then((url) => {
               ...
               this.newPostres.push(newPostre)
               console.log(this.newPostre)
               if(item.nombre == 'Cassata' || item.nombre == 'Tiramisú' || item.nombre == 'Tiramisú' || item.nombre == 'Frutillas y crema' || item.nombre == 'Brownie' || item.nombre == 'Don predro'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
