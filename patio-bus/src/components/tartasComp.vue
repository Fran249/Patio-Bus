<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="6">
               <v-row>
                   <v-col cols="6" v-for="(tarta, i) in tartas" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectTarta(tarta)">
                           <v-img :src="tarta.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
       ...
       <div class="selectores" >
           <div class="selector"  v-for="tarta in tartas" :key="tarta.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{tartas.length}}</h3>
   </div>
   </div>
   </v-container>
</transition>
 ...
 export default {
     ...
     methods: {
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','tartas',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.tartas = doc.data().pagina
               ...
               this.tartas.forEach(item =>{
               getDownloadURL(ref(storage, `Productos/tartas/${item.id}.jpg`))
               .then((url) => {
               const newTarta= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newTartas.push(newTarta)
               console.log(this.newTartas)
               if(item.nombre == 'Caprese'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','tartas','pagina2','pagina');
       setDoc(docRef,{pagina: this.tartas});
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','tartas','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
