<template>
    ...
    <v-container class="card-container" v-if="show">
              ...
              <v-col cols="6">
                <v-row>
                    <v-col cols="6" v-for="(sandwich, i) in sandwiches" :key="i">
                        <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectSandwich(sandwich)">
                            <v-img :src="sandwich.url">
                            </v-img>
                            <v-card-title>
                                <h3 class="title-cafe">
                                    {{sandwich.nombre}}
        ...
        <div class="selectores" >
            <div class="selector"  v-for="sandwich in sandwiches" :key="sandwich.id"></div>
        </div>
        <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{sandwiches.length}}</h3>
    </div>
    </div>
    </v-container>
</transition>
...
export default {
    ...
    methods: {
        ...
        cambiarPagina(){
            ...
            const docRef = doc(db, 'Productos','sandwiches',`pagina${this.index}`,'pagina')
        onSnapshot(docRef, (doc) => {
     

                this.sandwiches = doc.data().pagina
                ...
                this.sandwiches.forEach(item =>{
                getDownloadURL(ref(storage, `Productos/sandwiches/${item.id}.jpg`))
                .then((url) => {
                const newSandwich= {
                    nombre : item.nombre,
                    id : item.id,
                ...
                this.newSandwiches.push(newSandwich)
                console.log(this.newSandwiches)
                if(item.nombre == 'Sandwich de pollo'){
                    const newSelected = {
                        nombre: item.nombre,
                        id: item.id,
                        url: url,
                ...
                this.sandwiches = this.newSandwiches
        ...
        consolear(){
        const docRef = doc(db, 'Productos','sandwiches','pagina3','pagina');
        setDoc(docRef,{pagina: this.sandwiches});
    ...
    beforeMount(){
        const docRef = doc(db, 'Productos','sandwiches','pagina1','pagina')
        onSnapshot(docRef, (doc) => {
