<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="6">
               <v-row>
                   <v-col cols="6" v-for="(aperitivo, i) in aperitivos" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectAperitivo(aperitivo)">
                           <v-img :src="aperitivo.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
                                   {{aperitivo.nombre}}
       ...
       <div class="selectores" >
           <div class="selector"  v-for="aperitivo in aperitivos" :key="aperitivo.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{aperitivos.length}}</h3>
   </div>
   </div>
   </v-container>
</transition>
...
export default {
   ...
   methods: {
       ...
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','aperitivos',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.aperitivos = doc.data().pagina
               ...
               this.aperitivos.forEach(item =>{
               getDownloadURL(ref(storage, `Productos/aperitivos/${item.id}.jpg`))
               .then((url) => {
               const newAperitivo= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newAperitivos.push(newAperitivo)
               console.log(this.newAperitivos)
               if(item.nombre == 'Gancia' || item.nombre == 'Fernet' || item.nombre == 'Tia Maria' || item.nombre == 'Belnder'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
               ...
               this.aperitivos = this.newAperitivos
       ...
       consolear(){
       const docRef = doc(db, 'Productos','aperitivos','pagina4','pagina');
       setDoc(docRef,{pagina: this.aperitivos});

       },
       selectAperitivo(aperitivo){
           this.selected = [aperitivo]
           console.log(this.selected)
       },
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','aperitivos','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
