<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="6">
               <v-row>
                   <v-col cols="6" v-for="(bebida, i) in bebidas" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectBebida(bebida)">
                           <v-img :src="bebida.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
                                   {{bebida.nombre}}
       ...
       <div class="selectores" >
           <div class="selector"  v-for="bebida in bebidas" :key="bebida.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{bebidas.length}}</h3>
   </div>
   </div>
   </v-container>
</transition>
...
export default {
   ...
   methods: {
       ...
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','bebidas',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.bebidas = doc.data().pagina
               ...
               getDownloadURL(ref(storage, `Productos/bebidas/${item.id}.jpg`))
               .then((url) => {
               const newBebida= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newBebidas.push(newBebida)
               console.log(this.newBebidas)
               if(item.nombre == 'Aguas saborizadas x500ml' || item.nombre == 'Gaseosas x500ml' || item.nombre == 'Imperial x473ml'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','bebidas','pagina3','pagina');
       setDoc(docRef,{pagina: this.bebidas});
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','bebidas','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
