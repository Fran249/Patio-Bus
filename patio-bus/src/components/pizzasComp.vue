<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="6">
               <v-row>
                   <v-col cols="6" v-for="(pizza, i) in pizzas" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectPizza(pizza)">
                           <v-img :src="pizza.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
       ...
       <div class="selectores" >
           <div class="selector"  v-for="pizza in pizzas" :key="pizza.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{pizzas.length}}</h3>
   </div>
   </div>
 ...
 export default {
     ...
     methods: {
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','pizzas',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.pizzas = doc.data().pagina
               ...
               getDownloadURL(ref(storage, `Productos/pizzas/${item.id}.jpg`))
               .then((url) => {
               const newPizza= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newPizzas.push(newPizza)
               console.log(this.newPizza)
               if(item.nombre == 'Mozzarella' || item.nombre == 'Crudo y rucula'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','pizzas','pagina3','pagina');
       setDoc(docRef,{pagina: this.pizzas});
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','pizzas','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
       ...
       getDownloadURL(ref(storage, `Productos/pizzas/${item.id}.jpg`))
           .then((url) => {
               ...
               this.newPizzas.push(newPizza)
               console.log(this.newPizzas)
               if(item.nombre == 'Mozzarella'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
