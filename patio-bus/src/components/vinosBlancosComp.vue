...
export default {
   ...
   methods: {
       ...
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','vinosBlancos',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
               ...
               getDownloadURL(ref(storage, `Productos/vinosBlancos/${item.id}.jpg`))
               .then((url) => {
               const newVino= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               if(item.nombre == 'New Age' || item.nombre == 'Lopez x750ml'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','vinosBlancos','pagina2','pagina');
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','vinosBlancos','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
