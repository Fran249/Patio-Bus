<template>
   ...
   <v-container class="card-container" v-if="show">
             ...
             <v-col cols="12" md="6" xl="6">
               <v-row>
                   <v-col cols="6" v-for="(carne, i) in carnes" :key="i">
                       <v-card style="border-radius: 1px; border: 1px solid black " class="cardone" @click="selectCarne(carne)">
                           <v-img :src="carne.url">
                           </v-img>
                           <v-card-title>
                               <h3 class="title-cafe">
       ...
       <div class="selectores" >
           <div class="selector"  v-for="carne in carnes" :key="carne.id"></div>
       </div>
       <h3 class="mt-2" v-for="select in selected" :key="select.id">{{ select.id }} de {{carnes.length}}</h3>
   </div>
   </div>
   </v-container>
</transition>
 ...
 export default {
     ...
     methods: {
       cambiarPagina(){
           ...
           const docRef = doc(db, 'Productos','carnes',`pagina${this.index}`,'pagina')
       onSnapshot(docRef, (doc) => {
    

               this.carnes = doc.data().pagina
               ...
               this.carnes.forEach(item =>{
               getDownloadURL(ref(storage, `Productos/carnes/${item.id}.jpg`))
               .then((url) => {
               const newCarne= {
                   nombre : item.nombre,
                   id : item.id,
               ...
               this.newCarnes.push(newCarne)
               console.log(this.newCarnes)
               if(item.nombre == 'Lomito' || item.nombre == 'Costeletas (vacuno)'){
                   const newSelected = {
                       nombre: item.nombre,
                       id: item.id,
                       url: url,
       ...
       consolear(){
       const docRef = doc(db, 'Productos','carnes','pagina2','pagina');
       setDoc(docRef,{pagina: this.carnes});
   ...
   beforeMount(){
       const docRef = doc(db, 'Productos','carnes','pagina1','pagina')
       onSnapshot(docRef, (doc) => {
